...
[root_dir, record_name] = autoLoadLTEFile(base_dir, 'LTE', '20260825', 2);

% 旧 .bin
% dataFile = LegacyLTEDataFile(fullfile(root_dir, [record_name '.bin']));
% 单条 SigMF
...
% r_mask = -20 < r_lim & r_lim < 280;
r_mask = true(size(r_lim));
v_res = c / fc * (1 / 5e-4 / N_symbol) / 2;
v_lim = (N_symbol/2:-1:-N_symbol/2+1) * v_res;
% v_mask = -30 < v_lim & v_lim < 30;
v_mask = true(size(v_lim));
...
% full_frame = zeros(nfft, N_symbol*7);
% raw_csi_frame = zeros(N_crs_freq, N_symbol, nRx, nTx);
crs_frame = zeros(N_crs_freq, N_symbol);
wave_frame = zeros(lteSamplesPerSubframe*N_symbol/2, 1);
% data_power = [];
% data_max = [];
% side_power = [];
...
for sfIdx = 1:numSubframes
    ...
    % full_frame(:, time_idx) = full_grid;
    time_idx = mod((sfIdx-1)*2:sfIdx*2-1, N_symbol) + 1;
    % raw_csi_frame(:, time_idx, :, :) = CSI_G1;
    crs_frame(:, time_idx) = rxsubframe(5:6:end, 1:7:end, 1) ./ CSI_G1(:, :, 1, 1);
    time_idx = mod(sfIdx-1, N_symbol/2) * lteSamplesPerSubframe + 1;
    wave_frame(time_idx:time_idx+lteSamplesPerSubframe-1) = rxWaveform;
    % data_power = [data_power vecnorm(rxsubframe).^2];
    % data_max = [data_max max(reshape(rxWaveform(1:2192*14), [2192 14]), [], 1)];
    % side_power = [side_power vecnorm(side_lobe).^2];
    ...
    if time_idx(2) == N_symbol
        ...
        cir = fftshift(ifft(csi_chunk, [], 1), 1);
        rd_map = fftshift(fft(cir(r_mask, :), [], 2), 2);
        rd_mag = 20*log10(abs(rd_map(:, v_mask)) + 1e-6);
        ...
        % imagesc(v_lim(v_mask), r_lim(r_mask), rd_mag, display_range);
        set(gca, 'YDir', 'normal');
        colormap('jet');
        colorbar;
