...
function [spectrumScope,synchCorrPlot,pdcchConstDiagram] = hSIB1RecoveryExamplePlots(channelFigure,varargin)
    ...
    if (nargin==2)
        ...
        spectrumScope.Name = 'Received signal spectrum';
        spectrumScope.SampleRate = sr;
        spectrumScope.PlotMaxHoldTrace = true;
        spectrumScope.PlotMinHoldTrace = true;
        spectrumScope.ShowGrid = true;
        if (repositionPlots)
            spectrumScope.Position = plotPositions(1,:);
        end
